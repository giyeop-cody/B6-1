...
`PASS`는 현재 환경에서 실제 확인한 것만 사용한다. AWS 계정, Docker daemon, 외부 URL이 필요한 항목은 `PENDING` 또는 `SKIP`으로 둔다.
...
| IAM 최소권한 정책 구조 | PASS | 관리자 wildcard Allow 없음, 서울 외 EC2 Deny |
| AWS ValidateTemplate | PENDING | AWS 계정 없음 |
| Stack CREATE_COMPLETE | PENDING | AWS 계정 없음 |
| 외부 사이트와 `/health` 200 | PENDING | AWS 계정 없음 |
...
| 실제 리소스 삭제·Billing | PENDING | AWS 계정 없음 |
