...
| 과거 배포·삭제의 실행 주체 | CloudTrail 조회 권한 부족으로 추가 확인 불가 | 17-final-audit-dom.txt |

## 현재 상태

웹사이트는 배포·기능 검증 후 삭제됐다. 이번 점검에서도 프로젝트 스택과 위 리소스의 정리 상태를 확인했다. 중단할 EC2가 남아 있지 않아 신규 삭제 작업은 수행하지 않았다. 이전 IP는 현재 서비스 주소가 아니다.

IAM 사용자로 콘솔과 CloudShell을 사용하고 실제 리소스 조회를 수행한 증거는 확보했다. 과거 배포 전체의 실행 주체에 관한 판정은 이번 조회 결과로 확정하지 않는다. 기존 시점별 원본 기록은 유지한다.

## 남은 확인

- CloudTrail `LookupEvents` 권한이 있는 세션에서 과거 스택 생성·변경·삭제 이벤트의 `userIdentity` 확인.
