...
처음에는 Security Group과 IAM이 둘 다 "AWS에서 무언가를 막는 것"처럼 보여 섞어서 이해했다. 나눠 본 결과 Security Group은 인스턴스 앞에서 네트워크 패킷을 거르는 장치이고, IAM은 사람이나 프로그램이 어떤 AWS 작업(API)을 부를 수 있는지 정하는 장치다. 그래서 Security Group에 80번 포트를 열어 두어도 AWS를 조작할 권한이 생기는 것이 아니고, 반대로 IAM 권한이 아무리 넓어도 네트워크 경로가 막혀 있으면 서버에 접속할 수 없다. 둘은 서로 다른 층이라 각각 따로 설정하고 확인해야 한다.
...
Docker를 쓰지 않고 User Data에서 `dnf install nginx`로 직접 설치하는 방법도 따져 보았다. 설치 과정은 단순해지지만 로컬에서 확인한 환경과 EC2 환경이 갈라지고, 이미지로 버전을 고정할 수 없으며, 재부팅 후 자동 시작과 상태 확인을 OS 설정으로 따로 만들어야 했다. 로컬과 EC2에서 같은 결과를 먼저 확인해 두고 싶어 Docker를 선택했고, 그 대가로 이미지와 레이어 개념을 새로 익혀야 했다.
